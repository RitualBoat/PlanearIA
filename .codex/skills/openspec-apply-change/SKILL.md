...
1. **Select the change**
   ...
   - If ambiguous, run `npm exec --yes=false -- openspec list --json` to get available changes and use the **AskUserQuestion tool** to let the user select
...
3. **Get apply instructions**
   ...
   ```bash
   npm exec --yes=false -- openspec instructions apply --change "<name>" --json
   ```
   ...
   **Handle states:**
   - If `state: "blocked"` (missing artifacts): show the list of missing artifacts, open a follow-up issue to track them, and pause the apply
