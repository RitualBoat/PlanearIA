...
Primero el ViewModel con sus pruebas: los destinos, la cascada y la garantia de que toda ejecucion encola. Despues la hoja, con sus estados de cargando, vacio, error y sin conexion ya disenados. Luego la correccion del servicio viejo, con dos pruebas escritas contra el sintoma real: que la asignacion sobreviva a una sincronizacion y que asignar un entregable de verdad lo cambie. Despues la adopcion en la pantalla de Asignar recurso, que pierde sus tres ventanas emergentes y su pantalla de exito falsa. Cierran la validacion tecnica, la QA visual en cinco anchos con la prueba real de perder y recuperar conexion, y la revision adversarial.

## Resumen integral del change

Este cambio no construye motor de sincronizacion: conecta el que ya existe y funciona. Agrega una hoja y un ViewModel, y modifica dos archivos. Su valor no es tener un selector bonito: es que hoy la app le dice al docente que asigno su material y despues se lo quita en silencio. Eso erosiona la confianza en una app cuya promesa central es funcionar sin conexion. Al terminar, asignar sera el mismo gesto en cualquier pantalla, con confirmacion clara, y lo que el docente asigne se quedara asignado, con o sin internet. Ademas abre el camino que Office, Clases y Conecta necesitan en las olas siguientes, sin que ninguno tenga que copiarlo.
