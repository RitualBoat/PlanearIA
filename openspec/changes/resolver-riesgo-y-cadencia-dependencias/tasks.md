...
- [x] 5.5 Assessment `kind: remediation` capturado: `debt-control capture: PASS ... resolved:debt-6c9672a48059, resolved:debt-d73a5844fae3`; result `clean`, sin candidatos nuevos. `npm run debt:check` PASS; plan `preparacion-operativa-sdd-harness` activo 1/5 unidades (solo queda `debt-2887d890144e`, Ola 3).
- [x] 5.6 `npm run openspec:ready:archive -- --change resolver-riesgo-y-cadencia-dependencias --run-local`: change-root, tldr, brownfield-baseline, archive-manifest, proposal-traceability, surface-profile, validation-ids, evidence, rollback, adversarial-review, local-openspec-strict, local-harness-parity, local-opsx-patch, debt-assessment y debt-gate en PASS. `readiness.json` completo.
