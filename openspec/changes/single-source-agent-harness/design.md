...
### D6. Cuerpo canonico de los workflows opsx = el de Claude, sin comando zombi
El cuerpo mas completo (usa `AskUserQuestion`, checkmarks `>`, refiere `/opsx:archive`) es el de `.claude/commands/opsx/*`. Se toma como canonico en `.agents/workflows/opsx/*`, se elimina la referencia al comando continue inexistente, y las divergencias por harness (Codex decia "ask the user directly", frontmatter distinto) se reconstruyen como transformaciones del adaptador. **Antes de consolidar** se diffean linea por linea las 5 copias de `archive`/`explore`/`propose`/`sync` (no auditadas linea a linea en §9) para no perpetuar divergencias no detectadas.
