...
export function renderHandoff({ config, registry, evaluation, planId, repo = null, branchHint = null }) {
  ...
  const issueLine = issueNumbers.length
    ...
    : 'PENDIENTE: aun no existe issue de saneamiento sincronizado; ejecuta debt-control sync.';
  ...
  lines.push(
    ...
    '- Antes de archivar: capturar assessment con debt-control capture (kind: remediation) y pasar el gate de archive.',
    '- Verificacion de estado: debt-control check --json; la reanudacion exige todas sus condiciones en verde.',
    '',
    '## Rollback',
    '',
    ...
    '- Al cumplirse, debt-control check reporta el plan activo y el trabajo pausado puede retomarse.',
  );
  return lines.join('\n');
}
