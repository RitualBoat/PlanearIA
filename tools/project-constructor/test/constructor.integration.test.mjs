...
const sourceCli = path.join(packageRoot, "bin", "project-constructor.mjs");
const installedCliRelative = path.join(
  ".project-constructor",
  "runtime",
  "bin",
  "project-constructor.mjs",
);
...
async function runInstalled(target, command, extra = []) {
  return runConstructor(path.join(target, installedCliRelative), command, target, extra);
}
...
before(async () => {
  ...
  seedInstalledCli = path.join(baselineRoot, installedCliRelative);
}, { timeout: 120_000 });
...
test("bootstrap prepara un repositorio Git vacío e instala un runtime autocontenido", async () => {
  ...
  assert.equal(await exists(seedInstalledCli), true);
  assert.equal(
    await exists(path.join(baselineRoot, ".project-constructor", "runtime", "blueprint", "manifest.json")),
    true,
  );
...
test("schemas JSON 2020-12 compilan en strict y validan artefactos reales", async () => {
  ...
  const cases = [
    ...
    [
      ...
      ".project-constructor/runtime/blueprint/manifest.json",
    ],
    [
...
test("segundo bootstrap, sync --check y doctor se ejecutan desde el runtime instalado", { timeout: 120_000 }, async () => {
...
test("una colisión preexistente aborta antes de cualquier escritura parcial", { timeout: 120_000 }, async () => {
  ...
  const response = await runConstructor(seedInstalledCli, "bootstrap", target);
...
test("un fallo inyectado deja journal y la reanudación converge al estado limpio", { timeout: 120_000 }, async () => {
  ...
  const resumed = await runConstructor(seedInstalledCli, "bootstrap", target);
...
test("sync renderiza MCP local y remoto con shapes exactos en los cinco harnesses", { timeout: 120_000 }, async () => {
  ...
  const installedHarness = await import(pathToFileURL(path.join(
    target,
    ".project-constructor",
    "runtime",
    "src",
...
test("estado futuro se rechaza sin mutación y estado antiguo compatible migra a la versión vigente", { timeout: 120_000 }, async () => {
  ...
  assert.equal(JSON.parse(await readFile(legacyStatePath, "utf8")).stateFormatVersion, 1);
